...
(function() {
  ...
  app.Record = Backbone.Model.extend({
    ...
    url: function() {
      return '/tests/'+ (this.isNew() ? '' : this.id +'/');
    }
  });
  ...
  app.ResultsView = Backbone.View.extend({
    el: '#results-table',
    template: _.template( $('#tmpl-testResults-table').html() ),
    initialize: function() {
    ...
    render: function() {
      ...
      if (this.collection.length === 0) {
        $('#results-rows').append( $('#tmpl-testResults-empty-row').html() );
      }
    }
  });
  ...
  app.ResultsRowView = Backbone.View.extend({
    tagName: 'tr',
    template: _.template( $('#tmpl-testResults-row').html() ),
    events: {
